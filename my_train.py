...
def train(dataset, model_instance, args, same_feat=True,
    ...
    optimizer = torch.optim.Adam(
        filter(lambda p: p.requires_grad, model_instance.parameters()), lr=0.01
    )
    iter = 0
    ...
    plt.plot(epochs,train_loss, marker='o', color="#" + ''.join(\
                        [random.choice('0123456789ABCDEF') for i in range(6)]), label='train_loss')
    plt.plot(epochs,val_loss, marker='o', color="#" + ''.join(\
                    [random.choice('0123456789ABCDEF') for i in range(6)]), label='val_loss')
    plt.plot(epochs,test_loss, marker='o', color="#" + ''.join(\
                        [random.choice('0123456789ABCDEF') for i in range(6)]), label='test_loss')
    plt.xticks(epochs)
    plt.ylabel(" Train,Val, Test Losses")
    plt.xlabel("Number of epochs")
    plt.legend()
    plt.savefig('ckpt/test.png')
...
if __name__ == '__main__':
    ...
    data_loader = FUNSDCharGridDataLoaderBoxMaskBoxLabel(
        "./funsd_preprocess_train_word.pkl")
    data_loader_test = FUNSDCharGridDataLoaderBoxMaskBoxLabel(
        "./funsd_preprocess_test_word.pkl", data_loader.labels)
